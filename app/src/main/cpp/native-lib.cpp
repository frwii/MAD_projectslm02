...
// Updated runModel to accept model_path string
std::string runModel(const std::string& prompt, const std::string& model_path) {
    ...
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "runModel() started with path: %s", model_path.c_str());
    ...
    // USES THE DYNAMIC PATH FROM KOTLIN
    llama_model* model = llama_model_load_from_file(model_path.c_str(), model_params);

    if (!model) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Failed to load model from path");
    ...
    if (n_prompt <= 0) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Tokenization failed");
        llama_free(ctx);
        llama_free_model(model);
    ...
    long gen_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - t_gen_start).count();
...
// THE JNI FUNCTION: This must match your Kotlin 'external fun'
extern "C"
...
Java_edu_utem_ftmk_slm02_MainActivity_inferAllergens(
        ...
        jobject,
        jstring inputPrompt,
        jstring modelPath) { // <--- Added modelPath here
    ...
    // Call runModel with BOTH arguments
